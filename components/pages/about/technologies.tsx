  ...
  return (
    <>
      <GeneralContainer className="bg-stone-300 my-14">
        <h3 className="font-bold text-3xl">What are my Knowledge Sources?</h3>
        <p className="text-center">
          The main sources for my knowledge are <b>Youtube</b>, <b>Udemy</b> and reading <b>Docs</b>{' '}
          as well as reviewing code written for other's project.
          <br />
          Self-learning is exhausting and yet thrilling anytime I start a new road for new
          knowledge!
        </p>
      </GeneralContainer>
      <div className="text-center flex flex-col space-y-14 items-center my-10 text-slate-800">
        ...
        <div className="flex w-5/6 justify-between">
          {kbSources.map((kb) => (
            <div key={kb.name} className="flex flex-col items-center my-5">
              <img src={kb.logo} className="w-16 h-16 object-contain" alt={kb.name} />
              <p className="text-center text-2xl my-2 font-bold">{kb.name}</p>
            </div>
          ))}
        </div>
      </div>
